...
class WC_Wottica_Product
{
    public function __construct()
    {
        ...
        add_shortcode('wottica-lens-relation', [$this, 'shortcode_wottica_lens_relation']);
    }
    ...
    public function shortcode_wottica_lens()
    {
        ...
        echo "<a href='{$link}' class='button select-lens-btn'><div class='select-lens-row'>{$iconData} <span>Comprar Lente</span></div></a>";
    }

    public function shortcode_wottica_lens_relation()
    {
        $productId = get_the_ID();
        if (empty($productId)) {
            return '';
        }
        $link = home_url("/lentes-relation?id={$productId}");

        echo "<a href='{$link}' class='button select-lens-relation-btn'>Comprar Lente</a>";
    }
